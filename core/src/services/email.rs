    ...
    pub async fn send_email(
        ...
        if !self.config.enabled {
            return Err(AppError::ServiceUnavailable {
                service: "Email service is disabled".to_string(),
            });
        }
        ...
        let transport = self
            ...
            .ok_or_else(|| AppError::ServiceUnavailable {
                service: "SMTP transport not available".to_string(),
            })?;
    ...
    fn create_message(&self, to_email: &str, template: &EmailTemplate) -> AppResult<Message> {
        ...
        let message = if let Some(html_body) = &template.body_html {
            message_builder
                ...
                .map_err(|e| AppError::InvalidRequest {
                    message: format!("Failed to create multipart message: {}", e),
                })?
        } else {
            message_builder
                .body(template.body_text.clone())
                .map_err(|e| AppError::InvalidRequest {
                    message: format!("Failed to create message body: {}", e),
                })?
        };
    ...
    pub async fn test_connection(&self) -> AppResult<()> {
        ...
        transport
            ...
            .map_err(|e| AppError::ServiceUnavailable {
                service: format!("SMTP connection test failed: {}", e),
            })?;

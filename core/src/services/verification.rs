...
impl Default for VerificationConfig {
    fn default() -> Self {
        Self {
            ...
            dev_mode: std::env::var("RUST_ENV").unwrap_or_default() != "production",
...
impl VerificationStore {
    ...
    pub async fn store_code(
        ...
        if self.is_rate_limited(&email).await {
            ...
            return Err(ApiError::EmailAlreadySent);
        }
    ...
    /// Verify a code
    pub async fn verify_code(&self, email: &str, code: &str) -> Result<bool, ApiError> {
        let email = email.to_lowercase();
        ...
        if let Some(entry) = entries.get_mut(&email) {
            ...
            if entry.attempts > self.config.max_attempts {
                ...
                entries.remove(&email);
                return Err(ApiError::VerificationCodeInvalid);
            }
...
mod tests {
    ...
    async fn test_rate_limiting() {
        ...
        assert!(matches!(result.unwrap_err(), ApiError::EmailAlreadySent));
    }
    ...
    async fn test_max_attempts() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            ApiError::VerificationCodeInvalid
        ));
    }

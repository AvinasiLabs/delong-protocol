...
impl AiAuditService {
    ...
    pub async fn call_external_audit_service(
        ...
        let service_url = self
            .service_url
            .as_ref()
            .ok_or_else(|| AppError::Config("Config error".to_string()))?;
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| AppError::Internal(format!("Failed to call AI audit service: {}", e)))?;
        ...
        if !response.status().is_success() {
            ...
            return Err(AppError::Internal(format!("AI audit service returned error {}: {}", status, error_text)));
        }
        ...
        let audit_response: ExternalAuditResponse =
            response
                .json()
                .await
                .map_err(|e| AppError::Internal(format!("Failed to parse AI audit response: {}", e)))?;

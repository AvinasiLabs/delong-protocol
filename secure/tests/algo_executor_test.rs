...
use secure::{
    ...
    workers::algo_executor::{AlgoExecutor, ExecutorConfig},
};
use std::sync::Arc;
...
async fn test_execution_status_updates() {
    ...
    // Update to running
    let updated = AlgoExe::update_status(
        &db.pool,
        algo_exe.id,
    ...
    // Update to completed
    let completed = AlgoExe::update_completed(
        &db.pool,
        algo_exe.id,
...
async fn test_find_pending_executions() {
    ...
    let pending = AlgoExe::find_pending_to_run(&db.pool)
        .await
        .unwrap();
    ...
    let our_exes: Vec<_> = pending.iter()
...
async fn test_full_execution_flow() {
    ...
    let download_url = format!("https://codeload.github.com/{}/{}/tar.gz/{}", owner, repo, commit_hash);
    
    println!("Downloading algorithm from: {}", download_url);
    let response = reqwest::get(&download_url).await.unwrap();
    assert!(response.status().is_success(), "Failed to download from GitHub");
    ...
    if updated_exe.status == ExecutionStatus::Completed {
        assert!(updated_exe.result.is_some(), "Completed execution should have results");
        assert!(updated_exe.end_time.is_some(), "Completed execution should have end time");
